...
from .quant_utils import find_qlayers, add_actquant, ActQuantWrapper
from .utils import DEV, llama_down_proj_groupsize, cleanup_memory
#from .gptq_utils import gptq_fwrd, rtn_fwrd
...
def prune_magnitude(args, model, tokenizer, device=DEV, prune_n=0, prune_m=0):
...
def prune_wanda(args, model, tokenizer, device=DEV, prune_n=0, prune_m=0):
    use_cache = model.config.use_cache
    model.config.use_cache = False
...
@torch.no_grad()
def prune_sparsegpt(args, model, tokenizer, dev, prune_n=0, prune_m=0, device=DEV):
    ...
    print(f"inputs device: {inps.device}, outputs device: {outs.device}")
    ...
    if args.rotate:
        ...
        rotate_model(model, args, device)
        if args.reorder:
            permute(model, args, device, tokenizer)
            #rotate_model(model, args, device)
        add_actquant(model)  # Add Activation Wrapper to the model
        qlayers = find_qlayers(model)
        for name in qlayers:
